...
| Question  | Related Issues |
| :--- | :--- |
| What dates and deadlines do filers need to see to understand what data they need to submit and by when? | Issue |
| After a filer has submitted, what are the relevant actions and content to display?  | Issue |
| When filers are on the resolve errors and review warnings steps, is it better to have the "Continue filing" button text be descriptive instead (e.g. "Resolve errors" or "Review warnings")? | Issue |
...
| Question  | Related Issues |
| :--- | :--- |
| Can we still assume that anyone associated with the financial institution is authorized to submit the filing? Or do we need additional controls to ensure that only authorized individuals can sign and submit? | Issue |
| What do filers expect to see as a part of a submission receipt? | Issue |
| Does our revised language for parent entity reduce confusion when this doesn't apply to a financial institution? | Issue |
| Does adding dollar amount of loans and total number of loans to the "Confirm your register information" section help a filer feel more confident that they are submitting the correct data? | Issue |
| Who should receive the email confirmation after a filer submits a filing? Should it be only the filer, the filer and POC, anyone who is associated with the institution in our system? | Issue |

# Getting help
| Question  | Related Issues |
| :--- | :--- |
| Will there be any sort of data template that an institution can use? | Issue |
| Other than email, what types of support will we offer (chatbot, video tutorials, etc)? | Issue |
| How do we improve contextual help for things like RSSD ID and Parent Entity so that filers can learn what those things are and whether they are applicable? | Issue |
| Where are there gaps in helping resolve support requests? | Issue |

# Global design questions
| Question  | Related Issues |
| :--- | :--- |
| How do we handle multiple individuals working on the same filing? Especially when a filing is about to be submitted? | Issue |
| What translations will be available and on which pages? | Issue |
| How are we distinguishing the shared landing page (Home) from filing home (Filing) in a way that users expect? | Issue |
| What are the metrics we are using to judge the design's performance? | Issue |
| What do filers find useful in the header and footer? What is missing? | Issue |
| Where do filers expect to go when they click on the CFPB logo at the top of the page? | Issue |
